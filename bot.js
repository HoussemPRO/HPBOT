...
if (message.content.startsWith('!bcall')){
 if (message.author.id !== '540881222499565578') return message.reply('** هذا الأمر قفط لصاحب البوت و شكراًً **')
 if(!message.author.id === '540881222499565578') return;
...
m.sendMessage(args)
 ...
 client.on('message' , function (message){
      var token = 'NTQ0ODQ4MjExNDA4MzIyNTYw.D0hH5A.YEqGXL_zai8ZesyJCldURwKJ04M'; // التوكن هنا بس
      if(message.content === '!restart') {
if(message.author.id !== '540881222499565578') return message.reply('**الامر خاص بـ صاحب البوت وشكرا**');
          client.destroy();
...
client.channels.get("545952127629918209").send(`:white_check_mark: **${client.user.tag} دخل سيرفر جديد
...
client.on('guildDelete', guild => {
  client.channels.get("545952127629918209").send(`:negative_squared_cross_mark: **${client.user.tag} طلع من سيرفر
Server name: __${guild.name}__
Server owner: __${guild.owner}__
Server id: __${guild.id}__ 
Server Count: __${guild.memberCount}__**`)
...
if (ReBeLL.content.startsWith(`!8ball`)) {
...
client.on("message", message => {
   ...
   if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply('⚠ | **ليس لديك صلاحيات**');
      ...
      message.channel.sendMessage("", {embed: {
        ...
        footer: {
          text: "HP.bot" // غير هنا حط اسم البوت
        }
...
❖!bc ارسال رسالة لجميع اعضاء السيرفر
          ...
          .then(co => {
            text = co.first().content
              ...
              message.channel.send(`تم حفظ اقتراحك الرجاء انتضار الرد من قبل الاداره`)
                client.channels.get("546063278678343680").send(`${message.author.username}'s sug => ${text}`)
